...
type RecorderAction = {
  index: number;
  type: "click" | "fill" | "change" | "submit" | "navigation" | "tab";
  locator?: string | null;
  text?: string | null;
  value?: string | null;
  url?: string | null;
...
const serializeSession = (session: RecorderSession) => ({
  id: session.id,
    ...
    try {
      ...
      await context.exposeBinding("__qairaRecorderEmit", async (source, event: unknown) => {
        ...
        const normalizedType =
          type === "fill" || type === "change" || type === "submit" || type === "navigation" || type === "tab" ? type : "click";
