...
import { completeQueuedJob, executeQueuedApiStep, failQueuedJob, isQairaQueueConfigured, leaseNextQueuedJob, startQueuedJob } from "./qairaClient.js";
...
async function executeQueuedJob(job, workerId, logger) {
    ...
    try {
        ...
        for (const [index, step] of remainingSteps.entries()) {
            updateRunState(accepted.id, "running", `${job.test_case_title}: executing API step ${index + 1} of ${remainingSteps.length}.`);
...
export function startQueueWorker(logger) {
