...
const truncateText = (value, maxLength = 420) => {
...
const parseExpectedLiteral = (value) => {
    const normalized = normalizeText(value);
    if (!normalized) {
        return "";
    }
...
const normalizeKey = (value) => normalizeText(value).replace(/^@+/, "").toLowerCase();
const registerContextValue = (context, key, value) => {
...
const buildInitialContext = (envelope) => {
    const context = {
        values: {}
    };
...
const resolveContextValue = (context, scopedKey, fallbackScope = "t") => {
...
const interpolateText = (value, context) => String(value || "").replace(STEP_PARAMETER_PATTERN, (_match, scope, rawName) => {
...
const interpolateHeaders = (headers, context) => (headers || []).reduce((accumulator, header) => {
...
const buildResponsePreview = (bodyText, json) => {
    if (json !== null && json !== undefined) {
        return truncateText(stringifyValue(json));
    }
    return truncateText(bodyText);
};
const readJsonPath = (source, rawPath) => {
...
const buildApiEvidenceDataUrl = ({ status, title, url, method, captureLines }) => {
    ...
    const svg = [
        ...
        ...lines.map((line, index) => `<text x="52" y="${112 + index * 28}" font-size="${index === 0 ? 22 : 18}" font-family="Arial, sans-serif" fill="#16324f">${escapeXml(line)}</text>`),
        `</svg>`
    ].join("");
...
const escapeXml = (value) => value
    ...
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&apos;");
const buildCallbackSignature = (secret, payload) => createHmac("sha256", secret).update(payload).digest("hex");
const writeArtifactFile = async (relativePath, content) => {
    if (!relativePath) {
        return null;
    }
...
const emitCallback = async (envelope, payload) => {
...
const persistRunSummary = async (run, summary) => {
    await writeArtifactFile(run.artifact_bundle.summary_path, JSON.stringify(summary, null, 2));
};
const buildStepApiDetailMap = (stepSummaries = []) => stepSummaries.reduce((accumulator, summary) => {
    if (!summary?.step_id) {
        return accumulator;
    }
...
const summarizeAssertionProgress = (assertions) => `${assertions.filter((assertion) => assertion.passed).length}/${assertions.length} assertions passed`;
const buildApiAssertionResult = (validation, response) => {
...
const executeApiStep = async ({ envelope, run, step, context }) => {
    ...
    const assertions = (request.validations || [{ kind: "status", expected: "200" }]).map((validation) => buildApiAssertionResult(validation, {
    ...
    const assertionLines = assertions.map((assertion) => `${assertion.kind}${assertion.target ? `(${assertion.target})` : ""}: ${assertion.passed ? "passed" : `failed, expected ${assertion.expected || ""}, got ${assertion.actual || ""}`}`);
    ...
    await writeArtifactFile(stepArtifactPath, JSON.stringify({
        step_id: step.id,
        title: step.action || `Step ${step.order}`,
...
const markRunState = (runId, nextState, summary, extra) => updateRun(runId, (current) => ({
    ...current,
    ...extra,
    state: nextState,
    summary,
...
const buildFinalCallback = ({ envelope, run, stepOutcomes, stepSummaries, status, durationMs, errorMessage }) => ({
...
const buildProgressCallback = ({ envelope, run, stepOutcomes, stepSummaries, latestOutcome, durationMs, totalSteps }) => ({
...
const executeUnsupportedRun = async ({ envelope, run, logger, reason }) => {
...
async function executeRun(envelope, run, logger) {
        ...
        for (const step of orderedSteps) {
            ...
            if (progressRun) {
                ...
                if (result.outcome.status === "passed" && stepOutcomes.length < orderedSteps.length) {
                    try {
                        await emitCallback(envelope, buildProgressCallback({
                            envelope,
                            run: progressRun,
                            stepOutcomes,
                            stepSummaries,
            ...
            if (result.outcome.status === "failed") {
                ...
                await emitCallback(envelope, buildFinalCallback({
                    envelope,
                    run: failedRun,
                    stepOutcomes,
                    stepSummaries,
...
export function queueRunExecution(envelope, run, logger) {
    setImmediate(() => {
...
export function saveAcceptedRun(envelope, run) {
